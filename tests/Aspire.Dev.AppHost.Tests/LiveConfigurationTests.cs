...
{
    [Fact]
    public async Task ProductionConfiguration_ReferencesSecretsWithoutProvisioningTheirValues()
    {
        ...
        Assert.DoesNotContain(builder.Resources.OfType<ParameterResource>(),
            parameter => parameter.Name.StartsWith("live-", StringComparison.Ordinal));
        ...
        Assert.Equal(expectedSecrets.Count, environment.Keys.Count(key => key.StartsWith("Live__", StringComparison.Ordinal)));
        ...
        var assignment = Assert.Single(website.Resource.Annotations.OfType<RoleAssignmentAnnotation>());
