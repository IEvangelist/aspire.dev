...
- **Three breaking changes** — `RunAsEmulator` now selects the Linux-based vNext Cosmos DB emulator, Azure Front Door origin names now include the backend hostname, and the experimental terminal APIs moved from `Aspire.Hosting.Terminals` to `Aspire.Hosting.ApplicationModel`.
- …and much more.
