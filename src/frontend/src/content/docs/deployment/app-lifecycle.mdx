...
<Aside type="note">
  The TypeScript Docker Compose APIs use helper methods such as
  `composeFile.addVolume(...)` and `service.addVolume(...)` instead of
  constructing `Volume` objects directly.
</Aside>
...
<Aside type="note">
  You will need a GitHub personal access token to access the GitHub Container
...
<Aside type="note">
  The `--project` flag is needed when your AppHost isn't in the current
  directory.
</Aside>
